...
@Immutable
public class MyCsvReader extends FileReader implements DataReader<CsvReadOptions> {

  private static final MyCsvReader INSTANCE = new MyCsvReader();
  ...
  public String printColumnTypes(CsvReadOptions options) throws IOException {
    ...
    public String[][] getColumnNameAndTypes(CsvReadOptions options) throws IOException {
    Table structure = read(options, true).structure();
    ...
    String[][] column = new String[structure.rowCount()][];
    for (int r = 0; r < structure.rowCount(); r++) {
      String type = (String) structure.get(r, typeColIndex);

      String index = structure.getUnformatted(r, indxColIndex);


      String colName = structure.getUnformatted(r, nameColIndex);
      column[r] = new String[]{index,type,colName};
